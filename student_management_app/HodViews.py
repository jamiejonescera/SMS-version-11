...
def add_staff_save(request):
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            if 'teacher_license' in request.FILES:
                teacher_license = request.FILES['teacher_license']
                license = fss.save(teacher_license.name, teacher_license)
                teacher_license_url = fss.url(license)
            ...
            try:
                ...
                user.staffs.teacher_license = teacher_license_url
...
def edit_staff(request, staff_id):
    # Adding Staff ID into Session Variable
    ...
    if staff.address:
        address_parts = staff.address.split(', ')
    ...
    form.fields['employee_no'].initial = staff.employee_no
    form.fields['registration_no'].initial = staff.registration_no
    form.fields['registration_date'].initial = staff.registration_date
    ...
    if request.method != "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                staff_model.employee_no = employee_no
                staff_model.registration_no = registration_no
                staff_model.registration_date = registration_date

                if profile_pic_url is not None:
                    staff_model.profile_pic = profile_pic_url
                if teacher_license_url is not None:
                    staff_model.teacher_license = teacher_license_url
